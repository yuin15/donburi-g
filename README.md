...
音声対戦では、プレイヤーが明示的に時間延長を頼むと、試合中で未使用なら残り時間に関係なくサーバーが決定的に10秒延長します。この経路はOpenAI Responses APIの判定や応答待ちを使いません。低確率のライバル側からの延長提案は従来どおり残り15秒以内だけで、同意・拒否をOpenAI Responses APIの確定経路で扱います。CPU対戦のEXTENDカードも残り15秒以内のままです。
