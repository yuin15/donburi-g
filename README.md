...
2. 両者は$100から開始します。$1 / $3 / $5のBETを選び、**SPIN** をクリック、または **Space** で回転します。BETはいつでも変更でき、回転中の回転入力は予約されず、リール停止後の次の回転から受け付けます。$1は中央1ライン、$3は横3ライン、$5は横3ラインと斜め2ラインを判定します。
...
プレイヤーはリール停止後かつ1.1秒以上の間隔で受け付け、最大55回。ライバルは最大30回転し、通常はBET3、CPUが大きくリードするとBET1、終盤に負けているとBET5を選び、残高不足時は可能なBETへ下げます。現行9記号の3列、全729通りを列挙した理論還元率は各BET 81.481%です。1回の総配当上限はBET1が$30、BET3/$5が$39です。両者とも同じ基本リールを使い、改造はありません。詳しくは[ゲーム規則](docs/game-rules.md)。
