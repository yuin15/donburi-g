...
音声対戦では、発話を一つの会話ターンとして OpenAI Responses API が構造化判定します。プレイヤーからの$5要求、ライバルからの$5提案への同意、時間延長の要求・提案への同意は、サーバーが同意を確定して snapshot を送信してから音声で返答します。金銭は残高不足でも固定$5を移し、時間は新しい合意ごとに+10秒です。同じ提案への重複した「はい」は server 発行 offer ID で一度だけ扱います。通常のLive返答も同じ監査を通し、安全な雑談以外は元のPCMを流さず、確定台詞またはserver発行の提案に差し替えます。CPU対戦のEXTENDカードは従来どおり残り15秒以内です。

合意対象の通常音声は、字幕の遅延・更新を待つ有限の猶予と Responses 判定中だけ試合終了を保留します。判定の失敗・タイムアウト・新しい発話では未確定の元音声を捨てますが、すでに確定した移動や延長は取り消しません。ライバルの時間提案は、対応する提案音声が開始して終了するまで短い同意の対象になりません。
