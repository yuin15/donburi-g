...
describe('rival upgrade choice', () => {
  it('cancels in-flight reasoning when optional voice is stopped', async () => {
  ...
  it.each([
    ...
    { output_text: '' },
  ])('falls back for ambiguous or incomplete output: %j', async body => {
  ...
  it('keeps the match moving when the provider times out', async () => {
    vi.useFakeTimers();
    request.mockImplementation((_url: string, options: RequestInit) => new Promise((_resolve, reject) => {
      options.signal?.addEventListener('abort', () => reject(new Error('aborted')));
    }));
