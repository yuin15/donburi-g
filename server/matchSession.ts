...
import { acceptsImmediateLoanOffer, chooseLoanDecision, chooseRivalUpgrade, chooseTimeExtension, rejectsLoanOffer, requestsDirectLoan, requestsLoan, requestsTimeExtension } from './rivalBrain.js';
...
const DIRECT_LOAN_TRANSCRIPT_MAX_WAIT_MS = 1000;
...
export class MatchSession {
  ...
  private queueDirectLoanRequest(generation: number): void {
    ...
    const deadline = this.directLoanRequestSettle?.turn === turn
      ? this.directLoanRequestSettle.deadline
      : Date.now() + DIRECT_LOAN_TRANSCRIPT_MAX_WAIT_MS;
    ...
    this.scheduleDirectLoanRequest(turn, generation, deadline);
  }

  private scheduleDirectLoanRequest(turn: number, generation: number, deadline: number): void {
    const timer = setTimeout(() => {
      this.delegationSettles.delete(timer);
      if (this.directLoanRequestSettle?.timer !== timer) return;
      if (this.userSpeaking && Date.now() < deadline) {
        this.scheduleDirectLoanRequest(turn, generation, deadline);
        return;
      }
      this.directLoanRequestSettle = null;
    ...
    this.directLoanRequestSettle = { turn, generation, deadline, timer };
    this.delegationSettles.add(timer);
  }
  ...
  private settleDirectLoanRequest(turn: number, generation: number): void {
    ...
    if (this.closed || this.voiceDisabled || generation !== this.voiceGeneration || turn !== this.userSpeechTurn || this.loanDelegation || this.loanDecisionPending || this.directLoanRequestTurns.has(turn)) return;
