...
Previously, a player starting after an 80-second ready wait lost the game at the 120-second voice limit with 20 seconds left. The voice deadline now stops only optional audio/video while an active game keeps its original clock and scores. An unused lobby closes at 90 seconds, with the same deadline applied to a start request that arrives before a delayed timer runs. Thus the full game finishes before 150 seconds; voice teardown still starts by 120 seconds and is not extended by a late start. There is no new result voice connection after voice expires.

Focused lifecycle checks cover both audio-only and avatar modes: start after 80 seconds, cross the voice limit, accept another manual spin, and finish at 60 seconds with two player spins and 30 rival spins. Idle-timer and late-command checks also release resources once. These checks substitute providers and are not real API duration measurements.
