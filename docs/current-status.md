...
- 静止/非表示では連続描画せず、素材4点・概算20.4MiB。任意LiveClient/LiveKitはCPU入口で読み込まない。手動回転版の主JSは137.41KB gzip、CSSは5.37KB gzip。

## 確認した範囲

- 型検査、lint、203テスト、生成済みNode ESMの起動と拒否応答、本番ビルドが成功。実localhost WebSocket＋MatchSession結合2ケースは手動spin要求で確認した。認証・利用枠・外部プロバイダーは代替。500KB超のビルド警告は残る。
...
詳細: [手動回転の操作・公開確認](manual-spin.md)、[改造リールの表示](visible-upgrades.md)、[勝敗・改造の見栄え改善](duel-polish-verification.md)、[通信・期限の検証](live-sync-verification.md)、[ゲーム規則](game-rules.md)。[バランス測定](game-balance.md)は旧自動30回転の測定、[旧完了監査](completion-audit.md)はPR #34時点の履歴。

## 公開記録

- URL: https://slot-chan.vercel.app
- 公開コード: `65a21e34d6978bd23d6f2f1eef47b28fd5159fb0`（PR #66）。PR #65の両者の改造リール、PR #59の結果音声分離も含む。
- Vercel READY: `dpl_GmPiL3PNCQTp2BtuodXgbnvXTdYh`
- マージ後CI: https://github.com/yuin15/donburi-g/actions/runs/34678942012 （success）
- #63/#64もクローズ。主JS `index-DPJmBELS.js`、CSS `index-nEaOVh7L.css`。手動回転・連打予約・右側の改造・両者の回転と構成表示を公開済み。
