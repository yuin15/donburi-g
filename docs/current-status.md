...
2026-09-13。無料の60秒CPUスロット対戦が公開済み。**PC専用、横画面1280×720以上、マウス・キーボード**を対象にする。音声・映像は任意で、APIキー、招待、マイク、Upstashなしに遊べる。Issue #115 の残高・BET仕様はコード差分に反映されているが、この文書更新だけでは実画面・実機・公開版・Liveの動作を検証済みとは扱わない。
...
- MVVMを維持。Modelがリール停止からの3×3判定と時計、ViewModelが進行・BET・入力受付・表示状態、ViewがDOMとThree.jsを担当する。[構成](architecture.md)。
- #79でライバルを独立した自動回転へ変更。相手は2秒ごと、残高が続く範囲で最大30回。こちらが押さなくても進む。
- 両者は$30から開始。プレイヤーは$1/$3/$5のBETを選び、リール停止後かつ1.1秒以上の間隔で0–55回。回転中の入力は予約せず無視し、BET変更は進行中の回転を変えず次回から適用する。
- BETは毎回の回転で残高から差し引き、有効ラインのチェリー$3、ベル$6、7$30を合算して戻す。$1は中央、$3は横3本、$5は横3本と斜め2本。残高0は残り時間を観戦し、60秒時点の残高で勝敗を決める。
- #78で改造をいったん撤去。20/40秒の選択・予告・既定適用・構成変更・AI判断は発生せず、同じ基本リールで最後まで遊ぶ。#9のAI改造は対象外として閉じた。
- 現行9記号の全729停止組合せで各BETの理論還元率は81.481%。1回の総配当上限はBET1が$30、BET3/$5が$39。左右の回転・加点・残高反映を分け、60秒終了時は両者の最終停止を待つ。
- 結果に左右それぞれの回転数、絵柄別配当、最高の一回を表示。再戦で初期化し、詳細を開いても再戦ボタンを隠さない。
- 金色の3D筐体・4表情の架空ライバル・下向きリールを使用。Three.js描画は1つ。背景・表情の共有WebPは2点、回転する絵柄はHoudiniモデルから起動時に生成して両者で共有する。ゲーム中の配当アイコンも3Dモデル。入口・結果の小さな画像は別途残す。小リールの絵柄は正方形を保つ。
...
- #117で音声のみはPLAY時に最低95秒ぶんの対戦期限を確保し、接続から170秒以内に終了処理を開始する。未開始の音声ロビーは75秒、LiveAvatar映像は発行済み120秒トークン内で延長試合を完走させるため25秒で終了する。外部プロバイダー自身の切断は別途確認が必要。
...
- 過去の改造や両者同時回転の検証資料は、その当時の履歴。#115の実画面・実機・Live動作は未検証で、現在の操作・規則は[ゲーム規則](game-rules.md)を正とする。
