...
The timing and spin limits above describe a normal 60-second round; an extension moves the deadline. Offline CPU play can show text choices for fictional $5 transfers and a one-time **EXTEND** card with 15 seconds or less remaining. Live voice agreements are separate: each distinct extension agreement adds ten seconds, with no one-extension limit, and each agreed loan transfers a fixed $5 in either direction even if the lender's balance becomes negative. Server-issued turn and offer IDs deduplicate repeated representations of one agreement, including multiple affirmatives to the same offer. No currency has real-world value.
