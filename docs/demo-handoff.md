...
PCで公開ゲームを開き、PLAY NOW → BETを選択 → **SPIN** または Space → 60秒後に勝敗 → REMATCH。両者$30開始で、BET1/3/5はそれぞれ中央1本／横3本／横3本＋斜め2本を判定する。BETは毎回控除し、有効ラインの配当（チェリー$3、ベル$6、7$30）を合算する。回転中の入力は予約せず、残高0では残り時間を観戦する。ライバルは2秒ごとに回る。音声・映像なしでも遊べるが、#115仕様の実画面動作は未検証である。
