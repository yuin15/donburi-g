...
/** A confirmed mutual bonus always grants the same fixed amount to both sides. */
export const MUTUAL_BONUS_AMOUNT = 5;
...
export type GameEvent =
  ...
  | { type: 'mutual_bonus'; seq: number; at: number; before: MatchSnapshot; after: MatchSnapshot }
...
/** The authoritative fixed-$5 grant. An agreement, rather than a balance, authorizes it. */
export function grantMutualBonus(state: MatchState): Extract<GameEvent, { type: 'mutual_bonus' }> | null {
  if (state.status !== 'playing') return null;
  const before = getSnapshot(state);
  state.scores.player += MUTUAL_BONUS_AMOUNT;
  state.scores.rival += MUTUAL_BONUS_AMOUNT;
  return { type: 'mutual_bonus', seq: nextSeq(state), at: state.elapsed, before, after: getSnapshot(state) };
}
