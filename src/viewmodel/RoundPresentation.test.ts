...
describe('independent spin presentation', () => {
  it('keeps a purchase deducted when an older spin stops or is recovered', () => {
    ...
    presenter.syncPurchases(10);
    expect(presenter.scores.player).toBe(19);
    stopped[0]();
    expect(presenter.scores.player).toBe(22);
    presenter.syncPurchases(10);
    expect(presenter.scores.player).toBe(22);
    presenter.spin({ ...spin('player', 2, 26), payout: 0, upgradeSpent: 10 });
    stopped[1]();
