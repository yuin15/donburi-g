...
describe('independent spin presentation', () => {
  it('keeps a purchase deducted when an older spin stops or is recovered', () => {
    ...
    presenter.syncPurchases(15);
    expect(presenter.scores.player).toBe(14);
    stopped[0]();
    expect(presenter.scores.player).toBe(17);
    presenter.syncPurchases(15);
    expect(presenter.scores.player).toBe(17);
    presenter.spin({ ...spin('player', 2, 26), payout: 0, upgradeSpent: 15 });
    stopped[1]();
