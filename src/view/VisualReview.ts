...
export type ReviewExample = 'final-seconds' | 'extension-offered' | 'extension-accepted' | 'extension-rejected' | 'loan-rival-to-player' | 'loan-player-to-rival' | 'session-best' | 'mic-live' | 'mic-reply' | 'mic-muted' | 'mic-quiet' | 'normal' | 'small' | 'diagonal' | 'bell-cherry' | 'cherry-bell' | 'jackpot' | 'rival-jackpot' | 'both-jackpot' | 'quiet' | 'draw' | 'defeat' | 'final' | 'live-caption' | 'live-result-error' | 'live-result-closed' | 'rematch-ready';
...
export function mountVisualReview(port: ReviewPort): void {
  ...
  controls.innerHTML = `<details><summary>ローカル検収ツール（本番には含まれません）</summary><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">
    <button data-example="normal">通常</button><button data-example="final-seconds">残り8秒</button><button data-example="extension-offered">延長を提案</button><button data-example="extension-accepted">延長受諾</button><button data-example="extension-rejected">延長拒否</button><button data-example="loan-rival-to-player">貸借 RIVAL → YOU</button><button data-example="loan-player-to-rival">貸借 YOU → RIVAL</button><button data-example="session-best">自己ベスト・3連勝</button><button data-example="small">小当たり</button><button data-example="diagonal">斜め7</button><button data-example="bell-cherry">ベル／チェリー</button><button data-example="cherry-bell">チェリー／ベル</button><button data-example="jackpot">7揃い・逆転</button><button data-example="rival-jackpot">相手が7揃い</button><button data-example="both-jackpot">両者7揃い</button><button data-example="quiet">両者はずれ</button><button data-example="draw">引き分け</button><button data-example="final">最終スピン</button>
