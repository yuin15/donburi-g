...
export type ReviewExample = 'normal' | 'small' | 'jackpot' | 'rival-jackpot' | 'both-jackpot' | 'quiet' | 'draw' | 'defeat' | 'final' | 'live-caption' | 'live-result-error' | 'live-result-closed' | 'rematch-ready';
...
export function mountVisualReview(port: ReviewPort): void {
  ...
  const run = async (record: boolean) => {
    ...
    for (let i = 0; i < examples.length; i += 1) {
      ...
      port.snapshot({ matchId: 'visual-fixture', status: 'playing', elapsed: round * 2, remaining: 60 - round * 2, round, rounds: { player: round, rival: round }, scores: { player: total, rival: 0 }, stats: cloneMatchStats(matchStats), upgrades: { player: [], rival: [] }, eventSeq: i + 1 });
