...
export type WinSymbol = 'bell' | 'cherry';
...
export function createSymbolModels(environment: THREE.Texture): SymbolModels {
  ...
  const build = (source: string, kind: WinSymbol) => {
    ...
    new OBJLoader().parse(source).traverse(node => {
      ...
      const material = kind === 'bell' ? gold : node.name === 'cherry_fruit' ? fruit : plant;
  ...
  return {
    bell: build(bellSource, 'bell'), cherry: build(cherrySource, 'cherry'),
    dispose: () => { geometries.forEach(geometry => geometry.dispose()); [gold, fruit, plant].forEach(material => material.dispose()); },
  };
}
